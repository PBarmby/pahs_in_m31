...
"""
Purpose : Reads ISOCVF cube from the bulge and extrac a spectrum using the same aperture that we used for that region
...
Inputs  : This program reads two fits files.
          cvf82100606.fits - This is a IDL structure based cube of the nucleus.I used it just to get the wavelengths.
          bulge606.fits  - Same fits file but have converted so that it can be read in python.
...
def getplot(image):
    ...
    m1,m2,m3,m4,c1,c2,c3,c4 = squre(20,17,17,21,22,27,26,23) #Pixel coordinates of (x,y) fore each corner of the aperture.
...
def plotvaluesBul():
    image = fits.getdata('bulge606.fits')  # Reads the python friendly fits file
    isofits = fits.getdata('cvf82100606.fits')  # Reading the IDL based fits file
    wl = getwavelength(isofits) # Gets the wavelength range

    flux = getplot(image)
    flux = np.array([flux])
    wl = np.array([wl])
    flux = flux[wl < 16.0 ]  # Cuts the spectrum at 16 mu because that part was really noisy.
    wl = wl[wl < 16.0 ]

    # Saving the wavelengthand the flux into a file
    with open('isobulgetable', 'w') as f:
    ...
    iso1 = np.loadtxt("isobulgetable1")
    iso2 = np.loadtxt("isobulgetable2")

    iso2[0:,1] = iso2[0:,1] + 0.966  # 1st part is shifted up by 0.966
...
x,y,irs_wl,irs_flux = plotvaluesBul()
